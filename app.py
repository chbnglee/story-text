...
CR-6  CHUNK BOUNDARY MINIMIZATION.
      When a modifier (hedge, intensifier, degree adverb) modifies an
      adjacent chunk, keep them TOGETHER rather than inserting a boundary.
      WRONG:  "feels@@a little@@afraid"
      RIGHT:  "feels@@a little afraid"   OR   "feels a little afraid"
      Also applies to: "quite sad", "very tired", "so happy", "a bit scared"

CR-7  REPORTED SPEECH — ALL OR NONE.
      A reporting clause ("X says", "he calls", "she whispers") attached
      to direct speech must be treated CONSISTENTLY across the entire batch.
      Choose ONE rule and apply it to every instance:
        Option A (split):   "Bear!"@@he calls.   /   "Hi,"@@Liam says.
        Option B (no split): "Bear!" he calls.   /   "Hi," Liam says.
      Mixing treatments for the same structure is an error.

CR-8  PREPOSITIONAL PHRASE RULES.
      Separate a PP with @@ ONLY in these cases:
        ...
        (c) PP is a particle of a phrasal verb already identified as a chunk.
      DO NOT split generic locative / directional PPs:
        "under a bench", "from the shadows", "at the door",
        "into the room", "down the hill"  → no split.

CR-9  MAXIMUM SPLITS PER SENTENCE.
      Hard limit: floor(word_count / 4) @@ boundaries per sentence.
        6-word sentence  → max 1 boundary
        8-word sentence  → max 2 boundaries
       12-word sentence  → max 3 boundaries
      When over the limit, keep only the highest-confidence chunk splits
      (by property count and idiomaticity), and merge the rest.
...
CR-10 SELF-CHECK BEFORE OUTPUT.
      After generating all lines in the batch:
        1. Collect every lexical pattern that appeared more than once.
        2. Verify identical treatment across all occurrences.
        3. If inconsistent, apply the more frequent (or more conservative)
           treatment to ALL occurrences.
      Patterns to verify: all "X says/called" reporting clauses,
      all "look/feel/seem + Adj" patterns, all "is/was + Adj" patterns,
      all locative PPs, all hedge+modifier sequences.
...
Input:  SC07_ST01_D = The little bear feels a little afraid in the dark forest.
Output: SC07_ST01_D_C = The little bear feels@@a little afraid@@in the dark forest.
  → "a little afraid" = hedge+adj kept together (CR-6). Do NOT split further.
  → "in the dark forest" = generic locative PP → no split (CR-8).

Input:  SC08_ST01_N = "Bear!" he calls from the shadows.
Output: SC08_ST01_N_C = "Bear!"@@he calls@@from the shadows.
  → Reporting clause "he calls" separated from direct speech (CR-7, Option A).
  → "from the shadows" = generic locative PP → not split (CR-8).
  → ALL other "X says/calls" in this batch must use the same split rule (CR-7).

Input:  SC09_ST01_D = The man seems to be searching for something every day.
Output: SC09_ST01_D_C = The man seems to be@@searching for something@@every day.
  → 9 words → max 2 boundaries (CR-9). Two highest-confidence splits kept.
  → "every day" = COLLOCATION (CR-8b); "searching for something" = VERB_PATTERN.
