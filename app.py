...
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
CR-1  ABSOLUTE SUBJECT–VERB BOUNDARY BAN.
      NEVER place @@ between a subject and its verb.
      This rule has NO exceptions except: the verb is the first word
      of a fully validated formulaic chunk (FORMULAIC_UTTERANCE or
      FIXED_EXPRESSION) that begins the predicate.
      Before placing @@ after a subject, ask:
        "Is everything after the subject a single recognised chunk?"
        If NO → remove the @@.
      WRONG:  "Liam@@smiles gently at him"
      WRONG:  "Heavy rain@@begins to fall"
      WRONG:  "He@@hugs Bear"
      RIGHT:  "Liam smiles gently at him"   (no valid chunk follows)
      RIGHT:  "She fell asleep@@on the soft grass"  (chunk is BEFORE @@)

CR-2  @@ MARKS CHUNK EDGES ONLY.
      @@ indicates where a recognised lexical chunk begins or ends.
      Never insert @@ at clause boundaries, coordinating conjunctions,
      or any position that is not the start/end of a chunk.

CR-3  NO CHUNKS → NO SPLITS.
      If no span passes the ≥ 2-property threshold, return the sentence
      exactly as-is (no @@ at all).

CR-4  PRODUCTIVE GRAMMAR IS NOT A CHUNK.
      Syntactic patterns are not chunks unless independently validated.
      NOT chunks: "is happy", "is sad", "looks tired", "feels better",
                  "Liam draws a picture", "he hugs Bear"
      ARE chunks: "every day", "by the way", "take place", "run out of"

CR-4b AUXILIARY / TENSE / ASPECT CONSTRUCTIONS ARE NOT CHUNKS.
      Do NOT split any of these productive grammatical structures:
        be + V-ing  (progressive):  "is wearing", "is running"
        be + V-ed   (passive):      "is printed", "was found"
        modal + V:                  "could be", "will fall", "may come"
        V + to + V:                 "begins to fall", "wants to leave"
        seem/appear + to be + V-ing: "seems to be searching"
      WRONG: "is@@wearing a blue collar"
      WRONG: "begins@@to fall" / "begins to fall" (S/V split)
      RIGHT: "is wearing a blue collar" (no split; productive progressive)

CR-5  CONSISTENCY ACROSS SENTENCES.
      If pattern X is a chunk in one sentence, treat it identically in
      every other sentence of the same batch.

CR-6  CHUNK BOUNDARY MINIMIZATION.
      Hedges / intensifiers / degree adverbs that modify a chunk stay
      INSIDE the chunk — do not insert @@ between them and their head.
      WRONG:  "feels@@a little@@afraid"
      RIGHT:  "feels@@a little afraid"
      Applies to: "quite sad", "very tired", "so happy", "a bit scared"

CR-7  QUOTED SPEECH — ALWAYS SPLIT AT THE QUOTE BOUNDARY.
      Whenever a sentence contains a quoted string ("…"), ALWAYS place @@
      at the boundary between the quote and the surrounding clause.
      This applies to both directions:

        (a) Verb BEFORE quote  →  split between verb and quote:
              "It says@@\"Bear.\""
              "He whispers@@\"Come here.\""

        (b) Quote BEFORE reporting clause  →  split after the quote:
              "\"Hi, Bear,\"@@Liam says."
              "\"Bear!\"@@he calls."

      Apply this rule to EVERY quoted-speech sentence in the batch
      without exception — do not split some and leave others unsplit.

CR-8  PREPOSITIONAL PHRASE POLICY.
      ── Split with @@ BEFORE a PP only in these cases: ──
        (a) Fixed adverbial chunk: "as well", "at home", "in the end",
            "on time", "in fact", "for good", "by heart"
        (b) High-frequency manner/time collocation:
            "every day", "with delight", "with excitement", "in silence"
        (c) After the OBJECT of a phrasal verb:
            "looks for@@Bear"  (phrasal verb 'look for' + its object)
        (d) Clearly separable locative/temporal PP after a complete clause:
            "She fell asleep@@on the soft grass"
      ── Do NOT split ──
        • Generic locative PPs that are free combinations:
          "under a bench", "from the shadows", "at the door" → no split
        • PP internal to a fixed expression: "by the way" (no split inside)

      PHRASAL VERB DISAMBIGUATION:
        Ask: "Is [verb + preposition] a fixed phrasal unit?"
        Test: Can the preposition be freely swapped? If NO → phrasal verb.
          "look at Liam"  → 'look at' is phrasal → split: "looks at@@Liam"
          "find Bear under a bench" → 'find' not phrasal with 'under'
              → split: "find Bear@@under a bench"

CR-9  MAXIMUM SPLITS PER SENTENCE.
      Hard limit: floor(word_count / 4) @@ boundaries per sentence.
        6-word sentence  → max 1  |  8-word → max 2  |  12-word → max 3
      Over the limit: keep highest-confidence splits (property count +
      idiomaticity), merge the rest.
      WRONG (7 words, 3 splits): "The man@@seems to be@@searching for@@something"
      RIGHT (7 words, 1 split):  "The man seems to be searching for@@something"

CR-10 CONSISTENCY VERIFICATION PASS (run before final output).
      1. Group sentences by structural pattern:
           S+V  /  S+V+O  /  S+V+Adj  /  S+V+PP  /  S+V+O+PP
      2. Verify @@ placement is UNIFORM within each group.
      3. If inconsistent, apply the more conservative (fewer splits)
         treatment to ALL instances.
      4. Specifically verify: all reporting clauses, all "verb+Adj"
         patterns, all "be+V-ing/V-ed" constructions, all locative PPs,
         all hedge+modifier sequences.

CR-11 JUSTIFICATION CHECK (run before each @@ placement).
      Before writing any @@, state internally:
        "Which chunk does this boundary delimit, and which ≥ 2 properties
         does that chunk satisfy?"
      If the answer is "it feels like a pause" or "subject ends here"
      or "new phrase begins" → REMOVE the @@.
      Valid:   "@@ every day" — COLLOCATION: conventionalised + fixed
      Valid:   "look for @@ Bear" — PHRASAL_VERB: conventionalised + idiomatic
      Invalid: "Subject @@ Verb" — syntactic, not lexical
      Invalid: "be @@ V-ing"    — grammatical construction, not a chunk
...
EXAMPLES
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
── Chunk at predicate boundary (NOT at S/V boundary) ──
Input:  SC01_ST01_N = She fell asleep on the soft grass.
Output: SC01_ST01_N_C = She fell asleep@@on the soft grass.
  → "fell asleep" = COLLOCATION. @@ placed AFTER the chunk. "She" stays with verb (CR-1).

── No valid chunk → no split ──
Input:  SC02_ST01_N = Liam smiles gently at him.
Output: SC02_ST01_N_C = Liam smiles gently at him.
  → No span passes ≥ 2-property threshold (CR-3). S/V boundary never split (CR-1).

── Auxiliary construction: NEVER split ──
Input:  SC03_ST01_D = The dog is wearing a blue collar.
Output: SC03_ST01_D_C = The dog is wearing a blue collar.
  → "is wearing" = progressive (be+V-ing) → grammatical, not a chunk (CR-4b).

Input:  SC04_ST01_D = Heavy rain begins to fall from the sky.
Output: SC04_ST01_D_C = Heavy rain begins to fall@@from the sky.
  → "begins to fall" = V+to+V construction, not a chunk (CR-4b). No S/V split.
  → "from the sky" = locative PP separable after the complete clause (CR-8d).

── Modal construction not a chunk ──
Input:  SC05_ST01_D = Could it be a monster?
Output: SC05_ST01_D_C = Could it be a monster?
  → "Could it be" = modal+subject+verb → grammatical, not a chunk (CR-4b). No split (CR-3).

── Whole-sentence formulaic → no split ──
Input:  SC06_ST01_E = Take your time.
Output: SC06_ST01_E_C = Take your time.
  → Whole-sentence FORMULAIC_UTTERANCE → return as-is (whole-sentence check).
...
Output: SC07_ST01_N_C = By the way,@@there is no way of knowing@@what will@@take place.
  → "By the way" = FIXED_EXPRESSION; "there is no way of knowing" = FORMULAIC_UTTERANCE.

── Hedge + adj kept together (CR-6) ──
...
Output: SC08_ST01_D_C = The bear feels@@a little afraid@@in the forest.
  → "a little afraid" = hedge+adj, kept together (CR-6).
  → "in the forest" = generic locative PP; no split here (CR-8).

── Reporting clause + quoted speech ──
Input:  SC09_ST01_N = "Bear!" he calls from the shadows.
Output: SC09_ST01_N_C = "Bear!"@@he calls from the shadows.
  → Reporting clause split after direct speech (CR-7). "from the shadows" = generic PP, no split (CR-8).
  → ALL other "X says/calls" in this batch must receive the same treatment (CR-5, CR-10).

── Phrasal verb vs. independent PP disambiguation ──
Input:  SC10_ST01_D = The man seems to be searching for something every day.
Output: SC10_ST01_D_C = The man seems to be searching for@@something@@every day.
  → "seems to be searching" = auxiliary chain, not a chunk (CR-4b).
  → "searching for" = PHRASAL_VERB; boundary after phrasal verb before object.
  → "every day" = COLLOCATION (CR-8b). 10 words → max 2 boundaries (CR-9). ✓

── Productive S+be+Adj → no chunk, no split ──
Input:  SC11_ST01_N = Lily was happy and the sun was bright.
Output: SC11_ST01_N_C = Lily was happy and the sun was bright.
  → "was happy", "was bright" = S+be+Adj → NOT chunks (CR-4, CR-3).
