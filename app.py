...
CHUNK_SYSTEM_PROMPT = """\
You are applying tone-unit chunking to children's story sentences, \
following Lewis (1993)'s Lexical Approach principles.

CHUNKING RULES:
1. Each chunk should be a natural breath/pause unit, typically 2–5 words.
2. SPLIT at these boundaries:
   a. Before coordinating conjunctions (and / but / so / or) when preceded by a comma
   b. At subordinating conjunctions that begin a clause:
      when, because, if, although, while, before, after, since, as, until
   c. Between a main verb and its infinitive complement
      e.g.  "loves to@@change his colors",  "tries to@@run away"
   d. Before a long adverbial prepositional phrase (4+ words)
      e.g.  "in the middle of the forest",  "at the top of the hill"
   e. After an introductory phrase at the sentence start
      e.g.  "One sunny morning,@@he woke up."
3. KEEP TOGETHER — never split across:
   a. Article + noun  (the cat, a dog, an apple)
   b. Auxiliary + main verb  (was running, can jump, had eaten)
   c. Negative contractions  (don't, can't, won't, isn't, couldn't)
   d. Short verb + short object of ≤3 words  (loves him, holds her hand)
   e. Fixed phrases and idioms
4. SHORT SENTENCES (5 words or fewer total): return exactly as-is — no @@ at all.
5. Punctuation (comma . ! ?) stays attached to the word immediately before it.
6. Use @@ as the separator. NO spaces before or after @@.

EXAMPLE:
Input:  SC01_ST01_D = Milo is a little chameleon, and he loves to change his colors.
Output: SC01_ST01_D_C = Milo is a little chameleon,@@and he loves to@@change his colors.
...
# ═════════════════════════════════════════════════════════
with tab3:
    st.subheader("Step 3. Chunk & Vocab")
...
<div style="background:#e8f4fd; border-left:4px solid #4F81BD;
            padding:10px 14px; border-radius:4px; font-size:0.82rem; line-height:1.7;">
📌 Lewis (1993) Lexical Approach 기반 tone unit 분할 (Gemini API 사용)<br>
...
""", unsafe_allow_html=True)
    ...
    vcol_n, vcol_e, vcol_d = st.columns(3)
    with vcol_n:
